...
# Simple language detector for multilingual support
...
def is_multilingual(text):
    ...
    # Even lower threshold - if we detect any non-English language, it's multilingual
    non_english_languages = [lang for lang, score in languages if lang != 'english' and score > 0.3]  # Was 3.0, now 0.3
...
def extract_outline(pdf_path):
    ...
    # Modular heading detection function (improved with better thresholds)
    def is_heading(text, chars, bold_threshold=0.5, font_size=0, avg_font_size=12):
        ...
        is_german_vocab = (
            len(text) < 30 and  # Short vocabulary terms  
            ...
            (('(v)' in text or '(n)' in text or '(adj)' in text) or  # Part of speech indicators
             re.search(r'^[a-zäöüß]+ \([vna]', text.lower()) or  # German word with part of speech
             # Specific German vocabulary words that should be headings
             (len(text.split()) <= 3 and  # Max 3 words to avoid full sentences
              any(word in text.lower() for word in ['ausziehen', 'anprobieren', 'anziehen', 'nähen', 
                                                   'stricken', 'färben', 'flicken', 'zusammenpassen', 
                                                   'abstimmen', 'waschen'])) or
             # German phrases that are clearly vocabulary (not full sentences)
             re.search(r'^(einen?|eine|ein|sich|das|die|der) [a-zäöüß]+ (an|zu)?[a-zäöüß]*$', text.lower())
            )
        )
        ...
        for element in page_layout:
            if isinstance(element, LTTextContainer):
                for text_line in element:
                    ...
                    if not is_heading(text, chars, bold_threshold=0.3, font_size=size, avg_font_size=avg_font_size):
                        continue
    ...
    if first_pages_lines:
        ...
        for l in first_pages_sorted[:8]:  # Check first 8 largest elements
            ...
            if candidate_lower in table_headers:
                continue
            if any(header in candidate_lower for header in ["name age", "age relationship", "s.no name", "sno name", "relationship s.no"]):
                continue
            
            # Look for title-worthy content
    ...
    for l in lines:
        ...
        if txt_lower in table_headers:
            continue
        # Also exclude if text contains multiple table headers combined
        if any(header in txt_lower for header in ["name age", "age relationship", "s.no name", "sno name", "relationship s.no"]):
            continue
        ...
        if not is_heading(txt, chars, bold_threshold=0.3, font_size=l['size'], avg_font_size=doc_avg_font_size):
            continue
        if len(txt) > 60:
            continue
